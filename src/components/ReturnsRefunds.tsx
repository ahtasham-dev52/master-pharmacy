  ...
  return (
    <div className="space-y-6 font-sans text-xs">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center bg-slate-800 p-5 rounded-2xl border border-slate-700/40 gap-4">
        <div>
          <h1 className="text-xl font-bold text-white tracking-tight">Returns & Refund Ledger</h1>
          <p className="text-slate-400 text-xs mt-0.5">Log patient returns (refunds) or return expired/damaged wholesale stocks to distributors.</p>
        </div>
            ...
            <div className="bg-slate-800 rounded-2xl border border-slate-700/40 p-5 space-y-4 shadow-xl">
              ...
              <div className="flex gap-2">
                <input
                  ...
                  className="flex-1 bg-slate-900 border border-slate-700 rounded-lg p-2.5 text-white font-mono placeholder-slate-500 text-xs focus:outline-none focus:border-emerald-500"
                  placeholder="Enter invoice number (e.g. MP-2026-000001)"
                />
                <button
                  type="button"
                  onClick={handleSearchInvoice}
                  className="bg-slate-900 hover:bg-slate-700 border border-slate-700 text-emerald-400 px-4 py-2.5 rounded-lg font-bold transition-all cursor-pointer"
                >
                  Locate Invoice
                </button>
              </div>
              ...
              {foundSale && (
                <form onSubmit={handleCustomerReturnSubmit} className="space-y-4 pt-2">
                  <div className="bg-slate-900 p-3.5 rounded-xl border border-slate-700/40 space-y-1 font-mono text-[10px] text-slate-300">
                    <p>Original Invoice ID: <strong className="text-white">{foundSale.invoiceNumber}</strong></p>
                    <p>Customer: <strong className="text-white">{foundSale.customerName} ({foundSale.customerPhone})</strong></p>
                    <p>Net Paid: <strong className="text-white">{formatPKR(foundSale.netPayable)}</strong></p>
                  </div>

                  {/* Return items mapping */}
                  <div className="border border-slate-700/50 rounded-lg overflow-hidden text-slate-300 font-mono">
                    <table className="w-full text-left">
                      <thead className="bg-slate-900 text-[10px] text-slate-400">
                        <tr>
                      ...
                      <tbody className="divide-y divide-slate-800">
                        {returnItems.map((item, idx) => (
                          <tr key={item.saleItemId} className="hover:bg-slate-700/10 text-[11px]">
                            <td className="p-2.5 font-sans">
                              <p className="font-bold text-white">{item.brandName}</p>
                              <p className="text-[9px] text-slate-400">Batch {item.batchNumber}</p>
                            </td>
                            ...
                            <td className="p-2.5">
                              <select
                                ...
                                className="bg-slate-900 border border-slate-700 text-slate-300 rounded p-1 text-[10px]"
                              >
                  ...
                  <div className="space-y-1">
                    <label className="text-slate-400 font-semibold uppercase block">Mandatory Refund Reason *</label>
                    <textarea
                      required
                      value={custReturnReason}
                      onChange={(e) => setCustReturnReason(e.target.value)}
                      className="w-full h-16 bg-slate-900 border border-slate-700 rounded-lg p-2.5 text-white"
                      placeholder="e.g. Unopened blister strip returned by patient, verified by pharmacist."
                    />
                  </div>

                  <div className="flex justify-end gap-2.5 pt-2 border-t border-slate-700/50">
                    <button
                      type="button"
                      onClick={() => setFoundSale(null)}
                      className="bg-slate-900 hover:bg-slate-700 text-slate-300 font-bold px-4 py-2.5 rounded-lg cursor-pointer"
                    >
                      Cancel
                    </button>
                    <button
                      type="submit"
                      className="bg-emerald-500 hover:bg-emerald-600 text-slate-950 font-bold px-5 py-2.5 rounded-lg shadow-lg"
                    >
                      Approve Refund
                    </button>
                  </div>
            ...
            <div className="bg-slate-800 rounded-2xl border border-slate-700/40 p-5 space-y-4 shadow-xl">
              ...
              <div className="space-y-3.5 max-h-[350px] overflow-y-auto pr-1">
                {customerReturns.map(ret => (
                  <div key={ret.id} className="bg-slate-900/50 p-3.5 rounded-xl border border-slate-700/30 text-[10px] space-y-1 font-mono">
                    <div className="flex justify-between text-white font-bold text-xs font-sans">
                      <span>Refund for Inv# {ret.invoiceNumber}</span>
                      <span className="text-rose-400">{formatPKR(ret.refundAmount)}</span>
                    </div>
                    <p className="text-slate-400 font-sans mt-1">Reason: <span className="italic">"{ret.reason}"</span></p>
                    <p className="text-slate-500 text-[9px] pt-1 border-t border-slate-800">{new Date(ret.createdAt).toLocaleString('en-PK')}</p>
                  </div>
                ))}
                {customerReturns.length === 0 && (
                  <p className="text-slate-500 text-center py-6">No refunds logged in offline database.</p>
                )}
              </div>
            </div>
          </div>
            ...
            <div className="bg-slate-800 rounded-2xl border border-slate-700/40 p-5 space-y-4 shadow-xl">
              ...
              <form onSubmit={handleSupplierReturnSubmit} className="space-y-4 font-sans text-xs">
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  <div className="space-y-1">
                    <label className="text-slate-400 font-semibold uppercase block">Distributor *</label>
                    <select
                      required
                  ...
                  <div className="space-y-1">
                    <label className="text-slate-400 font-semibold uppercase block">Medicine *</label>
                    <select
                      required
                  ...
                  <div className="space-y-1">
                    <label className="text-slate-400 font-semibold uppercase block">Inventory Batch *</label>
                    <select
                      required
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-1">
                    <label className="text-slate-400 font-semibold uppercase block">Quantity to Return (Base Units/Tablets) *</label>
                    <input
                      type="number"
                      required
                      min="1"
                  ...
                  <div className="space-y-1">
                    <label className="text-slate-400 font-semibold uppercase block">Memo / Reason</label>
                    <input
                      type="text"
                ...
                <button
                  type="submit"
                  className="w-full bg-emerald-500 hover:bg-emerald-600 text-slate-950 font-bold py-3 rounded-lg text-xs uppercase tracking-wider transition-all cursor-pointer shadow-lg"
                >
            ...
            <div className="bg-slate-800 rounded-2xl border border-slate-700/40 p-5 space-y-4 shadow-xl">
              ...
              <div className="space-y-3 max-h-[350px] overflow-y-auto pr-1">
                {supplierReturns.map(ret => (
                  <div key={ret.id} className="bg-slate-900/50 p-3 rounded-xl border border-slate-700/30 text-[10px] space-y-1 font-mono">
                    <div className="flex justify-between text-white font-bold">
                      <span>{ret.supplierName}</span>
                      <span className="text-emerald-400">{formatPKR(ret.totalRefundAmount)}</span>
                    </div>
                    <p className="text-slate-500">Return logged on {new Date(ret.returnDate).toLocaleDateString('en-PK')}</p>
                  </div>
                ))}
                {supplierReturns.length === 0 && (
                  <p className="text-slate-500 text-center py-6">No supplier returns logged.</p>
                )}
              </div>
            </div>
          </div>
